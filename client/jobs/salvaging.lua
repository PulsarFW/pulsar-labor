...
function SetupTargetting()
	for k, v in ipairs(_models) do
		...
		plsr.Targeting:AddObject(v, "car", {
			{
				icon = "engine",
				text = "Scrap",
				...
				isEnabled = function(s, s2)
					return (_working and _inPoly and (_nodes ~= nil and not _nodes[s2.entity]))
				end,
			}
		}, 3.0)
	end
...
AddEventHandler("Labor:Client:Setup", function()
	plsr.Polyzone.Create:Poly(_POLYID, {
	...
	plsr.PedInteraction:Add("SalvagingJob", `s_m_y_construct_02`, vector3(2369.771, 3157.132, 47.209), 10.633, 25.0, {
		{
			icon = "helmet-safety",
			text = "Start Work",
		...
		},
		{
			...
			isEnabled = function()
				return plsr.State.character.ChopPickups ~= nil and #plsr.State.character.ChopPickups > 0
			end,
		},
		{
			icon = "list-timeline",
...
RegisterNetEvent("Salvaging:Client:OnDuty", function(joiner, time)
	...
	eventHandlers["startup"] = RegisterNetEvent(string.format("Salvaging:Client:%s:Startup", joiner), function()
		...
		if _inPoly or (not _inPoly and plsr.Polyzone:IsCoordsInZone(GetEntityCoords(PlayerPedId(), false, _POLYID, true)) and _count < 15) then
			SetupTargetting()
			_inPoly = true
		end
	end)
	...
	eventHandlers["delivery"] = RegisterNetEvent(string.format("Salvaging:Client:%s:StartDelivery", joiner), function(point)
		...
		_blip = plsr.Blips:Add("SalvDelivery", "Deliver Goods", point.coords, 478, 2, 1.4)

		plsr.PedInteraction:Add("SalvagingDelivery", `mp_m_waremech_01`, point.coords, point.heading, 25.0, {
			{
	...
	eventHandlers["actions"] = RegisterNetEvent(string.format("Salvaging:Client:%s:Action", joiner), function(entity)
		...
		if _count >= 15 then
			...
			if _blip ~= nil then
				RemoveBlip(_blip)
				plsr.Blips:Remove("SalvDelivery")
			end
		end
	end)
end)
...
AddEventHandler("Salvaging:Client:ScrapCar", function(s, s2)
	plsr.Progress:Progress({
		...
		if not cancelled then
			plsr.Callbacks:ServerCallback('Salvaging:SalvageCar', s.entity)
		end
	end)
end)

AddEventHandler("Salvaging:Client:TriggerDelivery", function()
    plsr.Callbacks:ServerCallback('Salvaging:TriggerDelivery', _joiner)
end)

AddEventHandler("Salvaging:Client:EndDelivery", function()
    plsr.Callbacks:ServerCallback('Salvaging:EndDelivery', _joiner)
	plsr.PedInteraction:Remove("SalvagingDelivery")
end)

AddEventHandler("Salvaging:Client:StartJob", function()
    plsr.Callbacks:ServerCallback('Salvaging:StartJob', _joiner, function(state)
		if not state then
