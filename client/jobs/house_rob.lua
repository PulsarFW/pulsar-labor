...
CreateThread(function()
	plsr.State.flags.inRobbedHouse = false
	plsr.State.flags.inRobbedHouse = false
end)
...
AddEventHandler("Labor:Client:Setup", function()
	...
	plsr.PedInteraction:Add("HouseRobbery", `csb_grove_str_dlr`, _queueLoc.coords, _queueLoc.heading, 25.0, {
		{
            ...
            data = {},
            isEnabled = function()
                return not hasValue(plsr.State.character.States or {}, "SCRIPT_HOUSE_ROBBERY") and plsr.State.flags.onDuty ~= "police"
            end,
		},
	}, 'question', 'WORLD_HUMAN_SMOKING')

	plsr.Callbacks:RegisterClientCallback("HouseRobbery:Lockpick", function(data, cb)
		_lpStage = 0
		DoLockpick({ timer = 1.0, base = 5 }, data, cb)
        plsr.EmergencyAlerts:CreateIfReported(40.0, "bane", true)
	end)

	plsr.Callbacks:RegisterClientCallback("HouseRobbery:AdvLockpick", function(data, cb)
		_lpStage = 0
		DoLockpick({ timer = 0.75, base = 8 }, data, cb)
        plsr.EmergencyAlerts:CreateIfReported(40.0, "bane", true)
	end)

	plsr.Interaction:RegisterMenu("house-robbery", "Enter House", "door-open", function(data)
        ...
        if _working and _p ~= nil and _nodes ~= nil then
            ...
            if robberyHouse then
                return #(vector3(plsr.State.flags.position.x, plsr.State.flags.position.y, plsr.State.flags.position.z) - vector3(robberyHouse.x, robberyHouse.y, robberyHouse.z)) <= 3.0
            end
        end
	...
	plsr.Interaction:RegisterMenu("house-robbery-exit", "Exit House", "door-closed", function(data)
		plsr.Interaction:Hide()
		ExitHouse()
	end, function()
        if GlobalState[string.format("%s:RobbingHouse", plsr.State.flags.ID)] ~= nil and _exit ~= nil then
            local dist = #(vector3(plsr.State.flags.position.x, plsr.State.flags.position.y, plsr.State.flags.position.z) - _exit)
            return dist <= 2.0
        else
            return false
        end
...
function DoAlarm(pId)
    ...
    _threading = true
    CreateThread(function()
        while _threading and _working and (not _nodes.states.alarm.disabled and not _nodes.states.alarm.triggered) and plsr.State.flags.inRobbedHouse do
            plsr.Sounds.Play:One("alarm_warn.ogg", 0.1)
            Wait(1000)
        end
        plsr.Sounds.Stop:One("alarm_warn.ogg")
        _threading = false
    end)
end

function ExitHouse()
    plsr.Callbacks:ServerCallback("HouseRobbery:Exit", {}, function(propId, intr)
        plsr.State.flags.inRobbedHouse = false
        ...
        plsr.Sounds.Stop:One("alarm_warn.ogg")
        plsr.Sounds.Stop:One("house_alarm.ogg", 0.1)
        ...
        if intr then
            for k, v in ipairs(intr.robberies.locations) do
                plsr.Targeting.Zones:RemoveZone(string.format("house-robbery-%s", k))
            end
        end

        if _stuff ~= nil then
            for k, v in ipairs(_stuff.pois or {}) do
                plsr.Targeting:RemoveEntity(v)
                DeleteObject(v)
            end
        end
        ...
        plsr.Polyzone:Remove("property-house-rob-zone")
    end)
end

AddEventHandler("Polyzone:Exit", function(id, testedPoint, insideZones, data)
	if id == "property-house-rob-zone" and plsr.State.flags.inRobbedHouse and not _isEntering then
...
function EnterHouse()
    if _p ~= nil and _working then
        plsr.Callbacks:ServerCallback("HouseRobbery:BreakIn", _p, function(r, f, exit)
            if r then
...
RegisterNetEvent("HouseRobbery:Client:OnDuty", function(joiner, time)
	...
	eventHandlers["receive"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Receive", joiner), function(propertyId, data)
		...
		if _blip then
            plsr.Blips:Remove("HouseRobbery")
        end
        _blip = plsr.Blips:Add("HouseRobbery", "Target House", { x = data.coords.x, y = data.coords.y, z = data.coords.z }, 40, 23, 0.9)

        CreateThread(function()
            local dist = #(vector3(plsr.State.flags.position.x, plsr.State.flags.position.y, plsr.State.flags.position.z) - vector3(data.coords.x, data.coords.y, data.coords.z))
            while _working and plsr.State.flags.loggedIn and dist > 20 do
                Wait(100)
                dist = #(vector3(plsr.State.flags.position.x, plsr.State.flags.position.y, plsr.State.flags.position.z) - vector3(data.coords.x, data.coords.y, data.coords.z))
            end
            plsr.Callbacks:ServerCallback("HouseRobbery:ArrivedNear", {})
        end)
	end)
	...
	eventHandlers["near"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Near", joiner), function(data)
        while _working and _p ~= nil do
            if plsr.State.flags.inRobbedHouse then
                Wait(100)
            else
                local dist = #(vector3(plsr.State.flags.position.x, plsr.State.flags.position.y, plsr.State.flags.position.z) - vector3(data.x, data.y, data.z))
                if dist < 20 then
	...
	eventHandlers["lockpicked"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Lockpicked", joiner), function(data)
        _nodes = data
        _state = 4
        plsr.Notification:Success("House Has Been Lockpicked")
	end)
	...
	eventHandlers["hacked"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:AlarmHacked", joiner), function(data)
        if _working and not _nodes.states.alarm.triggered and not _nodes.states.alarm.disabled and plsr.State.flags.inRobbedHouse then
	...
	eventHandlers["alarm-trigger"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:AlarmTriggered", joiner), function(data)
        if _working and not _nodes.states.alarm.triggered and not _nodes.states.alarm.disabled then
            ...
            if plsr.State.flags.inRobbedHouse then
                plsr.Sounds.Loop:One("house_alarm.ogg", 0.1)
            end
        end
	end)
        ...
        if data.id and not _nodes.searched[data.id] then
            plsr.Progress:ProgressWithTickEvent({
                ...
                if not cancelled then
                    plsr.Status.Modify:Add("PLAYER_STRESS", 3, false, true)
                    plsr.Callbacks:ServerCallback("HouseRobbery:Search", data.id)
                end
            end)
        end
    end)
    ...
    eventHandlers["inner"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:InnerStuff", joiner), function(intr)
        _isEntering = true
        while GlobalState[string.format("%s:RobbingHouse", plsr.State.flags.ID)] == nil do
            Wait(10)
            print("Interior Stuff Waiting, This Shouldn't Spam")
        end

        plsr.State.flags.inRobbedHouse = true
        ...
        if _nodes.chances.alarm then
            plsr.Targeting:AddEntity(o, "triangle-exclamation", {
                {
            ...
            elseif _nodes.states.alarm.triggered then
                plsr.Sounds.Play:One("house_alarm.ogg", 0.1)
            end
        else
            plsr.Targeting:AddEntity(o, "triangle-exclamation", {
                {
        ...
        if intr.zone then
            plsr.Polyzone.Create:Box("property-house-rob-zone", intr.zone.center, intr.zone.length, intr.zone.width, intr.zone.options, {})
        end
...
AddEventHandler("HouseRobbery:Client:Enable", function()
    plsr.Callbacks:ServerCallback('HouseRobbery:Enable', {})
end)

AddEventHandler("HouseRobbery:Client:StartJob", function()
    plsr.Callbacks:ServerCallback('HouseRobbery:StartJob', _joiner, function(state)
		if not state then
...
AddEventHandler("HouseRobbery:Client:HackSuccess", function(data)
	if not plsr.State.flags.inRobbedHouse then
		return
	end
	...
	if _scPass > 3 then
		_scPass = 1
        plsr.Callbacks:ServerCallback("HouseRobbery:HackAlarm", {
			propertyId = data.pId,
			state = true
		}, function()
            plsr.Notification:Success("Alarm Bypassed")
		end)
	else
		Wait(1500)
		DoHack(data.pId)
...
function DoLockpick(diff, data, cb)
	plsr.Minigame.Play:RoundSkillbar((diff.timer + (0.2 * data.tier)), diff.base - stageComplete, {
		onSuccess = function()
			Wait(400)
...
function DoHack(pId)
	plsr.Minigame.Play:Sequencer(5, 300, 7500 - (1000 * _scPass), 3 + _scPass, true, {
