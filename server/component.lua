...
AddEventHandler("Proxy:Shared:RegisterReady", function()
	exports["pulsar_core"]:RegisterComponent("Labor", _LABOR)
end)
...
_LABOR = {
	...
	Jobs = {
		Register = function(self, id, name, limit, salary, repAward, restrictions, customRep, hiddenRep, timeout)
			...
			plsr.Reputation:Create(id, name, customRep or {
		...
		Task = function(self, joiner, job, text, appOverride)
			...
			for k, v in pairs(_Jobs[job].OnDuty) do
				if v.Joiner == joiner then
					...
					plsr.Phone.Notification:AddWithId(joiner, "LABOR_OBJ", title, text, os.time(), -1, app, {})
				end
			end
		end,
						...
						if v.Joiner == joiner then
							if v.Group then
								for k2, v2 in pairs(_Groups) do
									if v2.Creator.ID == joiner then
										for k3, v3 in ipairs(v2.Members) do
											plsr.Phone.Notification:RemoveById(v3.ID, "LABOR_OBJ")
											if not paidOut[v3.ID] then
												paidOut[v3.ID] = true
												plsr.Labor.Offers:Complete(v3.ID, job)
											end
										end
									end
								end
							end
							plsr.Phone.Notification:RemoveById(joiner, "LABOR_OBJ")
							if not paidOut[joiner] then
								paidOut[joiner] = true
								plsr.Labor.Offers:Complete(joiner, job)
							end
							plsr.Labor.Duty:Off(job, joiner, true)
						...
						if v.Joiner == joiner then
							if v.Group then
								for k2, v2 in pairs(_Groups) do
									if v2.Creator.ID == joiner then
										for k3, v3 in ipairs(v2.Members) do
											plsr.Phone.Notification:Update(
												v3.ID,
												"LABOR_OBJ",
												title,
												string.format(
		...
		Complete = function(self, source, job)
			...
			if char then
				...
				if _Jobs[job].Salary > 0 then
					plsr.Banking.Balance:Deposit(plsr.Banking.Accounts:GetPersonal(char:GetData("SID")).Account, _Jobs[job].Salary, {
						type = "paycheck",
						title = "Paycheck",
				...
				plsr.Reputation.Modify:Add(source, job, _Jobs[job].RepAward)
			end
		end,
		ManualFinish = function(self, joiner, job, appOverride)
			...
			for k, v in pairs(_Jobs[job].OnDuty) do
				if v.Joiner == joiner then
					...
					plsr.Phone.Notification:RemoveById(v.Joiner, "LABOR_OBJ")
					if not paidOut[v.Joiner] then
						paidOut[v.Joiner] = true
						plsr.Labor.Offers:Complete(v.Joiner, job)
					end
					TriggerEvent(string.format("%s:Server:FinishJob", job), v.Joiner)
					plsr.Labor.Duty:Off(job, v.Joiner, true)
				end
			end
			...
			for k, v in pairs(_Jobs[job].OnDuty) do
				if v.Joiner == joiner then
								...
								for k3, v3 in ipairs(v2.Members) do
									...
									if not paidOut[v3.ID] then
										...
										if not _Jobs[job]?.Timeout?.KeepRep then
											plsr.Reputation.Modify:Remove(v3.ID, job, _Jobs[job].RepAward)
										end
									end
								end
							end
					...
					if not paidOut[v.Joiner] then
						paidOut[v.Joiner] = true
						if not _Jobs[job]?.Timeout?.KeepRep then
							plsr.Reputation.Modify:Remove(joiner, job, _Jobs[job].RepAward)
						end
					end
					TriggerEvent(string.format("%s:Server:CancelJob", job), joiner)
					plsr.Labor.Duty:Off(job, joiner, false, true)
				end
			end
		...
		Create = function(self, source)
			...
			if char ~= nil then
				...
				if hasValue(char:GetData("States") or {}, "PHONE_VPN") then
					local vpn = plsr.Inventory.Items:GetFirst(char:GetData("SID"), "vpn", 1)
			...
			for k, v in ipairs(_Groups) do
				if v.Creator.ID == source and (not _Groups[k].Working or force) then
					if _Groups[k].Working then
						plsr.Labor.Duty:Off(_Groups[k].Job, v.Creator.ID, false, true)
					end
					for k2, v2 in ipairs(v.Members) do
	...
	Duty = {
		...
		Off = function(self, job, joiner, wasFinished, noAlert)
			if _Jobs[job] ~= nil then
				-- OnDuty entries are only ever keyed to the workgroup creator's ID - a member calling
				-- this with their own ID would never match, so resolve members to their creator first
				local isDirectlyOnDuty = false
				for k, v in ipairs(_Jobs[job].OnDuty) do
					if v.Joiner == joiner then
						isDirectlyOnDuty = true
						break
					end
				end
				if not isDirectlyOnDuty then
					for k, v in ipairs(_Groups) do
						if v.Creator.ID ~= joiner then
							for k2, v2 in ipairs(v.Members) do
								if v2.ID == joiner then
									joiner = v.Creator.ID
									break
								end
							end
					...
					if v.Joiner == joiner then
						if v.Group then
							for k, v in ipairs(_Groups) do
								if v.Creator.ID == joiner then
									for k2, v2 in ipairs(v.Members) do
										local c = plsr.Fetch:CharacterSource(v2.ID)
										if c ~= nil then
											c:SetData("TempJob", nil)
											plsr.Phone.Notification:RemoveById(v2.ID, "LABOR_OBJ")
			...
			for k, v in pairs(_Jobs) do
				if not v.Restricted or not v.Restricted.state or v.Restricted.state ~= "SCRIPT_PRISON_JOB" then
					if v.Restricted then
						plsr.Labor.Offers:Fail(source, k)
					else
						plsr.Labor.Duty:Off(k, source, false, true)
					end
				end
			end
		end,
