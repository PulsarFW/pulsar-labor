...
AddEventHandler("Labor:Server:Startup", function()
	...
	plsr.Callbacks:RegisterServerCallback("Fishing:Catch", function(source, data, cb)
		...
		if char and data?.toolUsed and data?.zone and data?.difficulty and _fishingZoneBasicBait[data.zone] and (not _fishingCooldowns[source] or _fishingCooldowns[source] <= GetGameTimer()) then
			local toolUsedCount = plsr.Inventory.Items:GetCount(char:GetData("SID"), 1, "fishing_" .. data.toolUsed) or 0
			local correctBaitCount = plsr.Inventory.Items:GetCount(char:GetData("SID"), 1, _fishingZoneBasicBait[data.zone]) or 0
			local lootTable = {}
			...
			if toolUsedCount > 0 and data.difficulty > 0 then
				...
				if #filteredLoot > 0 then
					local lootItem = plsr.Loot:CustomWeightedSetWithCount(filteredLoot, 0, 0, true)
					if lootItem then
						if FishingConfig.FishItems[lootItem.name] then
							if char:GetData("TempJob") == _JOB and _joiners[source] ~= nil and _fishing[_joiners[source]] ~= nil and (_fishing[_joiners[source]].tool == data.toolUsed) then
								if plsr.Labor.Offers:Update(_joiners[source], _JOB, lootItem.count, true) then
							...
							if plsr.Inventory.Items:Has(char:GetData("SID"), 1, _fishingZoneBasicBait[data.zone], 1) then
								plsr.Inventory.Items:Remove(char:GetData("SID"), 1, _fishingZoneBasicBait[data.zone], 1)
							end
						end

						plsr.Inventory:AddItem(char:GetData("SID"), lootItem.name, lootItem.count, {}, 1)

						if lootItem.count > 0 then
							local hasToolItem = plsr.Inventory.Items:GetFirst(char:GetData("SID"), "fishing_" .. data.toolUsed, 1)
							local mult = 6
							...
							if hasToolItem then
								local toolData = plsr.Inventory.Items:GetData(hasToolItem.Name)
								local newValue = hasToolItem.CreateDate - (60 * 60 * mult * lootItem.count)
								if (os.time() - toolData.durability >= newValue) then
									plsr.Inventory.Items:RemoveId(char:GetData("SID"), 1, hasToolItem)
								else
	...
	plsr.Callbacks:RegisterServerCallback("Fishing:Sell", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)

		local itemData = plsr.Inventory.Items:GetData(data)

		if char and itemData then
			local count = plsr.Inventory.Items:GetCount(char:GetData("SID"), 1, itemData.name) or 0
			if (count) > 0 then
				if plsr.Inventory.Items:Remove(char:GetData("SID"), 1, itemData.name, count) then
					if Player(char:GetData('Source')).state.onDuty == 'police' or Player(char:GetData('Source')).state.onDuty == 'ems' then
						plsr.Execute:Client(source, "Notification", "Success", "Thanks for the donation! No money for you kek")
					else
						plsr.Wallet:Modify(source, itemData.price * count)
					end
				end
			else
				plsr.Execute:Client(source, "Notification", "Error", "You Have No " .. itemData.label)
			end
		end
	end)
end)
...
function RegisterFishingItems()
	plsr.Inventory.Items:RegisterUse("fishing_rod", "Labor", function(source, itemData)
		if _joiners[source] and _fishing[_joiners[source]] ~= nil and _fishing[_joiners[source]].state == 0 then
			...
			plsr.Labor.Offers:Start(_joiners[source], _JOB, "Catch Fish", 30)
			plsr.Labor.Workgroups:SendEvent(
				_joiners[source],
	...
	plsr.Inventory.Items:RegisterUse("fishing_net", "Labor", function(source, itemData)
		local repLvl = plsr.Reputation:GetLevel(source, _JOB)

		if repLvl < 3 then
			plsr.Execute:Client(source, "Notification", "Error", "Your Net is Tangled and You Don't Know What to Do...")
			return
		end
		...
		if _joiners[source] and _fishing[_joiners[source]] ~= nil and _fishing[_joiners[source]].state == 0 then
			...
			plsr.Labor.Offers:Start(_joiners[source], _JOB, "Catch Fish", 40)
			plsr.Labor.Workgroups:SendEvent(
				_joiners[source],
				string.format("Fishing:Client:%s:Startup", _joiners[source])
			)
	...
	plsr.Inventory.Items:RegisterUse("fishing_boot", "Fishing", function(source, item)
		local char = plsr.Fetch:CharacterSource(source)

		if char and plsr.Inventory.Items:Remove(char:GetData("SID"), 1, item.Name, 1) then
	...
	plsr.Inventory.Items:RegisterUse("fishing_chest", "Fishing", function(source, item)
		local char = plsr.Fetch:CharacterSource(source)
		if char and plsr.Inventory.Items:Remove(char:GetData("SID"), 1, item.Name, 1) then
			plsr.Loot:CustomWeightedSetWithCount({
...
AddEventHandler("Fishing:Server:OnDuty", function(joiner, members, isWorkgroup)
	...
	plsr.Labor.Offers:Task(joiner, _JOB, "Buy Fishing Equipment & Start Fishing")
	if #members > 0 then
