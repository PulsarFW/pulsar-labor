...
AddEventHandler("Labor:Server:Startup", function()
	...
	plsr.WaitList:Create("coke_import", "individual_time", {
		event = "Labor:Server:Coke:Queue",
		--delay = (1000 * 60) * 5,
		delay = 10000,
	})

	plsr.Callbacks:RegisterServerCallback("Coke:StartWork", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil then
			if plsr.Wallet:Has(source, 100000) then
				if not GlobalState["CokeRunActive"] and _active == nil then
					if not GlobalState["CokeRunCD"] or os.time() > GlobalState["CokeRunCD"] then
						plsr.Labor.Duty:On("Coke", source, true)
					else
						plsr.Execute:Client(source, "Notification", "Error", "Someone Has Already Done This Recently")
					end
				else
					plsr.Execute:Client(source, "Notification", "Error", "Someone Else Is Already Doing This")
				end
			else
				plsr.Execute:Client(source, "Notification", "Error", "You Don't Have Enough Cash, Come Back When You Do")
			end
		end
	end)

	plsr.Callbacks:RegisterServerCallback("Coke:Abort", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char ~= nil then
			if _active ~= nil and _active.joiner == source then
				if _active.state == 0 then
					plsr.Labor.Duty:Off("Coke", source, false, false)
					plsr.Wallet:Modify(source, 100000)
					...
					plsr.Execute:Client(source, "Notification", "Error", "Too Late, You Cannot Cancel This Now")
				end
			end
		end
	end)

	plsr.Callbacks:RegisterServerCallback("Coke:ArriveAtCayo", function(source, data, cb)
		if _joiners[source] ~= nil and _active.joiner == _joiners[source] and _active.state == 1 then
			_active.state = 2
			plsr.Vehicles:SpawnTemp(source, `squaddie`, 'automobile', vector3(4504.899, -4510.600, 4.367), 19.409, function(veh)
				plsr.Vehicles.Keys:Add(_joiners[source], plsr.State.Entity(veh).VIN)
				if _active.isWorkgroup then
					if #_active.members > 0 then
						for k, v in ipairs(_active.members) do
							plsr.Vehicles.Keys:Add(v.ID, plsr.State.Entity(veh).VIN)
						end
					end
				end
			end)
			...
			plsr.Vehicles:SpawnTemp(
				...
				function(veh)
					plsr.State.Entity(veh).Locked = false
					plsr.State.Entity(veh).noLockpick = true
					SetVehicleDoorsLocked(veh, 1)
					_active.entity = veh
					_active.VIN = plsr.State.Entity(veh).VIN
					plsr.Inventory:AddItem(_active.VIN, "coke_brick", 4, {}, 4)
					plsr.Labor.Workgroups:SendEvent(_joiners[source], string.format("Coke:Client:%s:GoTo", _joiners[source]))
				end
			)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("Coke:StartHeist", function(source, data, cb)
		if _joiners[source] ~= nil and _active.joiner == _joiners[source] and _active.state == 2 then
			_active.state = 3
			plsr.Labor.Offers:Task(_joiners[source], _JOB, "Locate The Target Vehicle", {
				title = "Unknown",
				label = "Unknown",
				icon = "question",
				color = "transparent",
	...
	plsr.Callbacks:RegisterServerCallback("Coke:ArrivedAtPoint", function(source, data, cb)
		if _joiners[source] ~= nil and _active.joiner == _joiners[source] and _active.state == 3 then
			_active.state = 4
			plsr.Labor.Offers:Task(_joiners[source], _JOB, "Retreive Contraband From Vehicle", {
				title = "Unknown",
				label = "Unknown",
				icon = "question",
				color = "transparent",
			})
			plsr.Labor.Workgroups:SendEvent(_joiners[source], string.format("Coke:Client:%s:DoShit", _joiners[source]))

			if not _active.pedsSpawned then
				_active.pedsSpawned = true
				plsr.Callbacks:ClientCallback(source, "Labor:Coke:GetSpawnCoords", _active.drop, function(coords)
	...
	plsr.Callbacks:RegisterServerCallback("Coke:LeftCayo", function(source, data, cb)
		if _joiners[source] ~= nil and _active.joiner == _joiners[source] and _active.state == 5 then
			_active.state = 6

			DeleteEntity(_active.entity)
			plsr.Vehicles:SpawnTemp(source, `bison`, 'automobile', vector3(1293.300, -3168.405, 4.906), 61.642, function(veh)
				plsr.State.Entity(veh).Locked = false
				plsr.State.Entity(veh).noLockpick = true
				SetVehicleDoorsLocked(veh, 1)
				_active.entity = veh
	...
	plsr.Callbacks:RegisterServerCallback("Coke:Finish", function(source, data, cb)
...
AddEventHandler("Inventory:Server:Opened", function(source, owner, type)
	if _joiners[source] ~= nil and _active.joiner == _joiners[source] and _active.state == 4 then
		if owner == _active.VIN and type == 4 then
			_active.state = 5
			plsr.Labor.Offers:Task(_joiners[source], _JOB, "Meet Contact Back In Los Santos", {
				title = "Unknown",
				label = "Unknown",
				icon = "question",
				color = "transparent",
			})
			plsr.Labor.Workgroups:SendEvent(_joiners[source], string.format("Coke:Client:%s:GoBack", _joiners[source]))
		end
	end
end)
...
AddEventHandler("Labor:Server:Coke:Queue", function(source, data)
	if _joiners[source] ~= nil and _active.joiner == _joiners[source] and _active.state == 0 then
		...
		plsr.Labor.Workgroups:SendEvent(_joiners[source], string.format("Coke:Client:%s:Receive", _joiners[source]))

		plsr.Labor.Offers:Task(_joiners[source], _JOB, "Speak To The Contact At Cayo Perico", {
			title = "Unknown",
			label = "Unknown",
			icon = "question",
			color = "transparent",
...
AddEventHandler("Coke:Server:OnDuty", function(joiner, members, isWorkgroup)
	if _active ~= nil then
		plsr.Phone.Notification:Add(joiner, "Unknown", "No Jobs Available", os.time(), 6000, {
			title = "Unknown",
			label = "Unknown",
			icon = "question",
			color = "transparent",
		...
		if char ~= nil then
			...
			if cd > os.time() then
				plsr.Phone.Notification:Add(joiner, "Unknown", "Your Group Is Not Eligible. Please Wait", os.time(), 6000, {
					title = "Unknown",
					label = "Unknown",
					icon = "question",
					color = "transparent",
		...
		plsr.Wallet:Modify(joiner, -100000)
		...
		plsr.Labor.Offers:Task(joiner, _JOB, "Wait For Contact", {
			title = "Unknown",
			label = "Unknown",
			icon = "question",
			color = "transparent",
		...
		plsr.WaitList.Interact:Add("coke_import", joiner, {
			joiner = joiner,
		})
	end
end)
...
AddEventHandler("Coke:Server:OffDuty", function(source, joiner)
	...
	plsr.WaitList.Interact:Remove("coke_import", source)
end)
